...
```mermaid
flowchart TD
    A[iRacing Simulator] -->|generates .ibt files| B[Python Agent]

    subgraph Agent
        B --> C[watchdog\nmonitors telemetry folder]
        C --> D[pyirsdk\nparses channels]
        D --> E[writer\nserializes to Parquet]
    end

    E -->|Parquet files| F[(DuckDB)]

    subgraph dbt
        F --> G[Bronze\nexternal tables]
        G --> H[Staging\nclean & typed]
        H --> I[Marts\nfct_laps · dim_track · dim_car]
    end

    I -->|orchestrated by Airflow| J[Power BI Dashboard]
```
